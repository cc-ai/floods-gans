...
class ResnetSetGenerator(nn.Module):
    def __init__(
        ...
        self.encoder_depth = self.get_encoder(
            1,
            n_downsampling,
            ngf,
            norm_layer,
        ...
        self.decoder_depth = self.get_decoder(
    ...
    def forward(self, inp):
        ...
        if self.use_depth:
            ...
            depth = inp[:, self.input_nc : self.input_nc + 1, :, :]  # (B, 1, W, H)
            segs = inp[:, self.input_nc + 1 :, :, :]  # (B, CA, W, H)
        else:
            img = inp[:, : self.input_nc, :, :]  # (B, CX, W, H)
        ...
        if self.use_depth:
            enc_depth = self.encoder_depth(depth)
        ...
        # run decoder
        if self.use_depth:
            feat = torch.cat([enc_img, enc_segs_sum, enc_depth], dim=1)
        else:
