...
def tf_normlize_to_one_score(scores):
    ...
    result = tf.reduce_sum(tf.stack(result, -1), axis=1, keep_dims=True)
    return result

def tf_accumulate(tensor):
    batch_list = tf.unstack(tensor)
    result = []
    for t in batch_list:
        channel_list = tf.unstack(t)
        for c in range(1, len(channel_list)):
            channel_list[c] = channel_list[c] + channel_list[c-1]
        result.append(tf.stack(channel_list))
...
def tf_photorealism_loss(img, df, i, is_our):
    ...
    if is_our:
        ...
        img_r = tf.concat(axis=0, values=[img_r, e])
        mat_t_r = df.csr_mat_r[i]
        mat_t_g = df.csr_mat_g[i]
        mat_t_b = df.csr_mat_b[i]
        img_r_b, img_r_g, img_r_r = tf.split(axis=1, num_or_size_splits=3, value=img_r)
        ...
        result = tf.reduce_mean(tf.stack([result_r, result_b, result_g])) / k
    else:
        mat_t = df.csr_mat[i]
...
def tf_clip_loss(img, ori, df, i):
    ...
    img_b = tf.where(img_t <  0, ori_t - img_t, img_o)
    img_b = tf.where(ori_t == 0,         img_o, img_b)
    img_f = img_b
...
def tf_comparison_loss(guess, label):
    ...
    label = tf.where(label > 0, label + shift, label - shift)
    weight_t = tf.where(label_zero_index, shift*tf.sign(-guess), label)
    return label * guess
...
def tf_crop_to_patch(inputs, labels, patch_size, channel, seed):
    ...
    for i in range(len(inputs_list)):
        ...
        concat_tensor = tf.concat(axis=2, values=[inputs_list[i], labels_list[i]])
        concat_tensor = tf.random_crop(concat_tensor, crop_size, seed=seed)
        split0, split1 = tf.split(axis=2, num_or_size_splits=2, value=concat_tensor)
        inputs_list[i] = split0
        labels_list[i] = split1

    return tf.stack(inputs_list), tf.stack(labels_list)
...
def tf_random_crop_resize(inputs, labels, img_size, channel, scale, seed):
    ...
    for i in range(len(inputs_list)):
        ...
        crop_size = tf.concat(axis=0, values=[crop_size, [channel * 2]])
        concat_tensor = tf.concat(axis=2, values=[inputs_list[i], labels_list[i]])
        concat_tensor = tf.random_crop(concat_tensor, crop_size, seed=seed)
