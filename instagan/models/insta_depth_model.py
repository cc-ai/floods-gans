...
class InstaGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument(
            "--set_order", type=str, default="decreasing", help="order of segmentation"
        )
        parser.add_argument(
            "--ins_max",
            type=int,
            default=4,
            help="maximum number of instances to forward",
        )
        parser.add_argument(
            "--ins_per",
            type=int,
            default=2,
            help="number of instances to forward, for one pass",
        )
        if is_train:
            parser.add_argument(
                "--lambda_A",
    ...
    def select_masks(self, segs_batch):
        ...
        if self.opt.set_order == "decreasing":
            ...
            raise NotImplementedError(
                "Set order name [%s] is not recognized" % self.opt.set_order
            )
    ...
    def select_masks_random(self, segs_batch):
        ...
        for segs in segs_batch:
            ...
            reorder = np.concatenate(
                (np.random.permutation(num), np.arange(num, self.opt.ins_max))
            )
    ...
    def forward(self, idx=0):
        N = self.opt.ins_per
        self.real_A_seg_sng = self.real_A_segs[
            :, N * idx : N * (idx + 1), :, :
        ]  # ith mask
        self.real_B_seg_sng = self.real_B_segs[
            :, N * idx : N * (idx + 1), :, :
        ]  # ith mask
        empty = -torch.ones(self.real_A_seg_sng.size()).to(self.device)  # empty image

        self.forward_A = (
            self.real_A_seg_sng + 1
        ).sum() > 0  # check if there are remaining instances
        self.forward_B = (
            self.real_B_seg_sng + 1
        ).sum() > 0  # check if there are remaining instances
        ...
        if self.forward_A:
            ...
            self.fake_B_mul = torch.cat(
                [self.fake_B_img_sng, self.fake_B_seg_mul], dim=1
            )

        # forward B
        if self.forward_B:
            self.real_B_sng = torch.cat(
                [self.real_B_img_sng, self.real_B_seg_sng], dim=1
            )
            ...
            self.fake_A_mul = torch.cat(
                [self.fake_A_img_sng, self.fake_A_seg_mul], dim=1
            )
    ...
    def backward_G(self):
        ...
        if self.forward_A:
            ...
            self.loss_cyc_A = (
                self.criterionCyc(self.rec_A_sng, self.real_A_sng) * lambda_A
            )
            self.loss_idt_B = (
                self.criterionIdt(
                    self.netG_B(self.real_A_sng), self.real_A_sng.detach()
                )
                * lambda_A
                * lambda_idt
            )
            ...
            self.loss_ctx_A = (
                self.weighted_L1_loss(
                    self.real_A_img_sng, self.fake_B_img_sng, weight=weight_A
                )
                * lambda_A
                * lambda_ctx
            )
        ...
        if self.forward_B:
            ...
            self.loss_cyc_B = (
                self.criterionCyc(self.rec_B_sng, self.real_B_sng) * lambda_B
            )
            self.loss_idt_A = (
                self.criterionIdt(
                    self.netG_A(self.real_B_sng), self.real_B_sng.detach()
                )
                * lambda_B
                * lambda_idt
            )
            ...
            self.loss_ctx_B = (
                self.weighted_L1_loss(
                    self.real_B_img_sng, self.fake_A_img_sng, weight=weight_B
                )
                * lambda_B
                * lambda_ctx
            )
