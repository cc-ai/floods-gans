...
try:
    os.mkdir(current_path + "/Downloaded")
except:
    pass
try:
...
def get_json_from_lat(lat, lng):
    ...
    URL = (
        "https://maps.google.com/cbk?output=xml&ll="
        + str(lat)
        + ","
        + str(lng)
        + "8&dm=1"
            ...
            for y in range(tile_height):
                for x in range(tile_width):
                    ...
                    if global_x < width and global_y < height:
                        pano_image[(global_y * width + global_x) * 3 + 0] = tile[
                            (y * tile_width + x) * 3 + 0
                        ]
                        pano_image[(global_y * width + global_x) * 3 + 1] = tile[
                            (y * tile_width + x) * 3 + 1
                        ]
                        pano_image[(global_y * width + global_x) * 3 + 2] = tile[
                            (y * tile_width + x) * 3 + 2
                        ]
...
def dl_panorama(pano_id, zoom_level):
    ...
    if (img[220:250, 0, :] == 0).all():
        ...
        cv2.resize(
            img_to_return,
            dsize=(512 * 2 ** zoom_level, 256 * 2 ** zoom_level),
...
def dl_cubic_img(lat, lng, dir_output):
    ...
    FACE_NAMES = {0: "back", 1: "left", 2: "front", 3: "right"}
