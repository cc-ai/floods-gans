...
class MUNIT_Trainer(nn.Module):
    ...
    def gen_update(self, x_a, m_A, x_b, m_B, hyperparameters):
        ...
        x_aba = self.gen_a.decode(c_ab, s_fBA, m_A, s_bAB) if hyperparameters['recon_x_cyc_w'] > 0 else None
        x_bab = self.gen_b.decode(c_ba, s_fAB, m_B, s_bBA) if hyperparameters['recon_x_cyc_w'] > 0 else None
        ...
        self.loss_gen_cycrecon_x_a = self.recon_criterion(im_A * x_aba, im_A*x_a) if hyperparameters['recon_x_cyc_w'] > 0 else 0
        self.loss_gen_cycrecon_x_b = self.recon_criterion(m_B* x_bab, m_B*x_b) if hyperparameters['recon_x_cyc_w'] > 0 else 0
        # GAN loss
    ...
    def sample(self, loader_a, loader_b, size):
        self.eval()
        im_a = torch.stack([loader_a.dataset[i][0] for i in range(size)]).cuda()
        seg_a = torch.stack([loader_a.dataset[i][1] for i in range(size)]).cuda()
        im_b = torch.stack([loader_b.dataset[i][0] for i in range(size)]).cuda()
        seg_b = torch.stack([loader_b.dataset[i][1] for i in range(size)]).cuda()
        ...
        for i in range(im_a.size(0)):
            ...
            if 0 == i%2:
                x_AB = (1 * (1-mask_a) * x_a + (0 * (1-mask_a) * x_AB)) + mask_a * x_AB
                x_BA = (1 * (1-mask_b) * x_b + (0 * (1-mask_b) * x_BA)) + mask_b * x_BA
...
class UNIT_Trainer(nn.Module):
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            mask_a = seg_a[i].unsqueeze(0)
            mask_b = seg_b[i].unsqueeze(0)
            x_a = x_a[i].unsqueeze(0)
            x_b = x_b[i].unsqueeze(0)
            ...
            # decode (cross domain)
            x_BA = self.gen_a.decode(c_b, s_fA, mask_b, s_bB)
            x_AB = self.gen_b.decode(c_a, s_bA, mask_a, s_fB)
